...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{x0}\NormalTok{:=-4}\KeywordTok{;}\ExtensionTok{y0}\NormalTok{:=0}\KeywordTok{;}
\ExtensionTok{with}\NormalTok{(stats)}\BuiltInTok{:}\NormalTok{ data:=[xx,yy,zz]: }
\ExtensionTok{fit1}\NormalTok{:=fit[leastsquare[[x,y,z], }
    \VariableTok{z=}\NormalTok{a1+a2}\ExtensionTok{*}\NormalTok{(x-x0)}\ExtensionTok{+a3*}\NormalTok{(y-y0)}\ExtensionTok{+a4*}\NormalTok{(x-x0)}\ExtensionTok{*}\NormalTok{(y-y0)}\ExtensionTok{+}
      \ExtensionTok{a5*}\NormalTok{(x-x0)^}\ExtensionTok{2+a6*}\NormalTok{(y-y0)^}\ExtensionTok{2}\NormalTok{, }
    \DataTypeTok{\{a1,a2,a3,a4,a5,a6\}}\NormalTok{]]}\KeywordTok{(}\ExtensionTok{data}\KeywordTok{)}\NormalTok{;}

\ExtensionTok{fit1}\NormalTok{ := z = .5026144708-0.4629870219e-2*x-0.7128049500e-2*y+}
\KeywordTok{(}\ExtensionTok{0.5266188957e-2*}\NormalTok{(x+4}\KeywordTok{)}\NormalTok{)}\ExtensionTok{*y+0.5395943477e-2*}\NormalTok{(x+4)^}\ExtensionTok{2+0.4563284219e-2*y}\NormalTok{^2}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{e1}\NormalTok{:=diff(f1(x,y),}\ExtensionTok{x}\NormalTok{);}
\ExtensionTok{e2}\NormalTok{:=diff(f1(x,y),}\ExtensionTok{y}\NormalTok{);}
\ExtensionTok{s1}\NormalTok{:=solve(}\DataTypeTok{\{e1,e2\},\{x,y\}}\NormalTok{);}
       \ExtensionTok{0.03853767760}\NormalTok{ + 0.005266188957 y + 0.01079188695 x}
      \ExtensionTok{0.01393670633}\NormalTok{ + 0.005266188957 x + 0.009126568438 y}
\NormalTok{              \{}\ExtensionTok{x}\NormalTok{ = -3.933335758, y = 0.7425554356\}}
\ExtensionTok{subs}\NormalTok{(s1,f1(x,y));}
                          \ExtensionTok{0.5183331423}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  df.to_s + }\CharTok{' '}\NormalTok{ + es.to_s + }\StringTok{"\textbackslash{}n"}
\KeywordTok{end}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ func(ax)}
  \DataTypeTok{$n_calc}\NormalTok{+=}\DecValTok{1}
  \DataTypeTok{$model}\NormalTok{.set_cell_size(ax)}
  \DataTypeTok{$model}\NormalTok{.total_energy+}\FloatTok{150.45633275921924}
\KeywordTok{end}

\CommentTok{# mnbrak, coding from Num Recipe in C}
\KeywordTok{def}\NormalTok{ mnbrak(ax, bx)}
\NormalTok{  printf(}\StringTok{"init range:%10.5f-%10.5f\textbackslash{}n"}\NormalTok{,ax,bx)}
  \DataTypeTok{$n_calc}\NormalTok{ = }\DecValTok{0}

\NormalTok{  fa = func(ax)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ func_x(ax,ay=}\FloatTok{1.0}\NormalTok{)}
  \DataTypeTok{$n_calc}\NormalTok{+=}\DecValTok{1}
  \DataTypeTok{$model}\NormalTok{.set_cell_size(ax,ay)}
  \DataTypeTok{$model}\NormalTok{.total_energy+}\FloatTok{150.45633275921924}
\KeywordTok{end}

\KeywordTok{def}\NormalTok{ func_y(ay, ax=}\FloatTok{1.0}\NormalTok{) }\CommentTok{# note the order of ax, ay }
  \DataTypeTok{$n_calc}\NormalTok{+=}\DecValTok{1}
  \DataTypeTok{$model}\NormalTok{.set_cell_size(ax,ay)}
  \DataTypeTok{$model}\NormalTok{.total_energy+}\FloatTok{150.45633275921924}
\KeywordTok{end}
...
\CommentTok{# mnbrak, coding from Num Recipe in C}
\KeywordTok{def}\NormalTok{ mnbrak(ax, bx, method_func=method(}\StringTok{:func_x}\NormalTok{))}
\NormalTok{  printf(}\StringTok{"init range:%10.5f-%10.5f\textbackslash{}n"}\NormalTok{,ax,bx)}
  \DataTypeTok{$n_calc}\NormalTok{ = }\DecValTok{0}

\NormalTok{  fa = method_func.call(ax)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  x_min =  mnbrak(}\FloatTok{1.0}\NormalTok{, }\FloatTok{1.02}\NormalTok{, method(}\StringTok{:func_x}\NormalTok{) )}
\NormalTok{  y_min =  mnbrak(}\FloatTok{1.0}\NormalTok{, }\FloatTok{1.02}\NormalTok{, method(}\StringTok{:func_y}\NormalTok{) )}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{math := <<   }\FloatTok{2.00000}\NormalTok{|   }\FloatTok{0.00000}\NormalTok{|   }\FloatTok{0.24140}\NormalTok{>,}
\NormalTok{<   }\FloatTok{0.00000}\NormalTok{|   }\FloatTok{0.00000}\NormalTok{|   }\FloatTok{0.00000}\NormalTok{>,}
\NormalTok{<  -}\FloatTok{3.23607}\NormalTok{|   }\FloatTok{0.00000}\NormalTok{|   }\FloatTok{1.11778}\NormalTok{>,}
\NormalTok{<   }\FloatTok{0.00000}\NormalTok{|   }\FloatTok{2.00000}\NormalTok{|   }\FloatTok{0.35218}\NormalTok{>,}
\NormalTok{<   }\FloatTok{0.00000}\NormalTok{|   }\FloatTok{0.00000}\NormalTok{|   }\FloatTok{0.00000}\NormalTok{>,}
\NormalTok{<   }\FloatTok{0.00000}\NormalTok{|  -}\FloatTok{3.23607}\NormalTok{|   }\FloatTok{0.37768}\NormalTok{>>;}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  print }\StringTok{"indicate additional calc point [x,y]:"}
\NormalTok{  line = gets}
\NormalTok{  x,y=line.chomp.split(}\CharTok{','}\NormalTok{)}
  \DataTypeTok{$model}\NormalTok{.set_cell_size(x.to_f/}\FloatTok{100.0}\NormalTok{+}\FloatTok{1.0}\NormalTok{,y.to_f/}\FloatTok{100.0}\NormalTok{+}\FloatTok{1.0}\NormalTok{)}
\NormalTok{  z = }\DataTypeTok{$model}\NormalTok{.total_energy+}\FloatTok{150.45633275921924}
\end{Highlighting}
\end{Shaded}
